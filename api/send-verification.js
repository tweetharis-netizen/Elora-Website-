...
function classifySmtpError(e) {
  ...
  if (code === "eauth" || msg.includes("invalid login") || msg.includes("username and password not accepted") || msg.includes("535")) {
    return "smtp_auth_failed";
  }
  if (code === "esocket" || code === "etimedout" || msg.includes("timeout")) return "smtp_timeout";
  if (msg.includes("getaddrinfo") || msg.includes("enotfound") || msg.includes("econnrefused")) return "smtp_connect_failed";
  if (msg.includes("missing") && msg.includes("smtp")) return "smtp_config_missing";
...
module.exports = async function handler(req, res) {
  ...
  // Hard requirement: KV must be used for rate limit + persistence
  try {
  ...
  // Link goes to FRONTEND so cookie gets set on the UI domain
  ...
  try {
    ...
    return json(res, 500, { ok: false, error: classifySmtpError(e) });
  }
};
